...
function MyJobs() {
        ...
        const getJobsPostedByMe = async () => {
            try {
                ...
                const response = await axios.get(
                    `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/v1/job/get-employer-posted-jobs`,
    ...
    const handleUpdateJob = async (id) => {
        ...
        axios.put(
            `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/v1/job/update/${id}`,
            updatedJob,
            {
                headers: {
    ...
    const handleDeleteJob = async (id) => {
        axios.delete(
            `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/v1/job/delete/${id}`,
            { withCredentials: true }
        ).then(() => {
                ...
                {myJobs.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {myJobs.map((element, index) => (
                            <div className="p-4 bg-slate-300 rounded-md shadow-lg flex flex-col" key={element._id}>
                                <div
                                    className="w-10 h-10 text-black rounded-full inline-flex items-center justify-center bg-red-500 text-xl font-bold mb-2">
                                    {index + 1}
                                </div>
                                <div>
                                    <div className="mb-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full sm:w-80 p-2 border-2 border-solid border-cyan-700 outline-none text-black rounded-md"
                                        />
                                    </div>
                                    <div className="mb-2">
                                ...
                                <div className="flex justify-end mt-4">
                                    {editingMode === element._id ? (
                                        <>
                                            <button
                                                ...
                                                className="p-2 bg-sky-500 rounded-md shadow-md font-bold text-white w-32 sm:w-40 m-2"
                                            >
                                                Save
                                            </button>
                                            <button
                                    ...
                                    <button
                                        ...
                                        className="p-2 bg-red-500 rounded-md font-bold text-white m-2 w-32 sm:w-40"
                                    >
                                        Delete
                                    </button>
                                </div>
                    ...
                    <p className="text-center">
                        {myJobs.length <= 0 ? "You've not posted any job or maybe you've deleted all of your jobs!" : ""}
                    </p>
                )}
            </div>
        </div>
